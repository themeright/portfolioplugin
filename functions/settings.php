...
class PortfolioPluginSettings  {
	...
	function addOptionsPage() {
		add_options_page('My Portfolio Setting Page', 'Portfolio Setting', 'manage_options', 'page-pp', array($this,'optionsPage'));
	...
	function optionsPage() { ?>
		...
		<div id = "portfolio-general"  class="wrap">
             <h2> My portfolio - Setting API </h2>
  ...
  return $catsArray;
   ...
   function portfolio_admin_init(){
		  ...
		  register_setting('portfolio_setting','portfolio_options',array($this,'portfolio_validate_options'));
          ...
          add_settings_field('category_choose', 'Choose category', array($this,'category_select_list'),'portfolio_setting_section','portfolio_main_section',
          ...
          add_settings_field('tag_filter_select','Tag Filter Select',array($this, 'tag_filter_select_check_box'),
  ...
  function tag_filter_check_box( $data = array()) {
        ...
        ?>
          <input type="checkbox" name="portfolio_options[<?php echo $name;  ?>]"
 ...
 function tag_filter_select_check_box($data = array() ) {
      ...
      // $tags = get_tags();
       ...
       foreach( $tags as $tag)
       {
          ...
          $checked = in_array($tag->tag_name, $options['tag_filter_select']) ? 'checked="checked"' : '';

            
          $html .= sprintf('<input type="checkbox" id="tag_filter_select[%1$s]" name="portfolio_options[%3$s][]" 
             value="%1$s" %2$s />', $tag->tag_name, $checked, $pag);
 ...
 function category_select_list( $data = array() ){
            ...
            <select name="portfolio_options[<?php echo $name; ?>]">  
